...
두 번째 구어체 예시는 `READY(배포)`, 세 번째 예시는 `NOT_READY(배포)`를 목표로
만든다. `통과했어`, `완료했어`, `아직이야`, `해도 돼?`처럼 상태와 질문이 명시된
일상 표현만 고정밀 규칙으로 받는다. `아마 곧 승인될 거야`처럼 가능성만 말한 문장은
상태 fact로 만들지 않고 `unparsed_statements`에 남긴다.

`테스트는 통과했어`처럼 서술어가 전제 이름의 일부를 대신하는 경우에는 이미 선언된
전제 중 정확히 `테스트_통과`가 있을 때만 그 이름에 맞춘다. 선언된 typed 전제가 없는
새 이름을 만들어 proof에 넣지는 않는다.
...
현재 회귀 테스트는 라벨, 한국어·영어 문장, 격식체·구어체 상태, 허용 질문, 부정,
미충족 전제, 모순, 애매한 문장, 휴리스틱 격리, 기존 graph projection을 포함한다.
